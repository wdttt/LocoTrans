...
from model_equi import IECNN_cls, knn
import numpy as np
...
def train(args, io):
    ...
    # Try to load models
    if args.model == 'iecnn':
        model = IECNN_cls(args).to(device)
    else:
    ...
    for epoch in range(args.epochs):
        ...
        for data, label in tqdm(train_loader):
            ...
            opt.zero_grad()
            logits, logits_vn, logits_fuse, [vector,idx] = model(data)
            loss_1 = criterion(logits, label)
            loss_2 = criterion(logits_vn, label)
            loss_3 = criterion(logits_fuse, label)
            ...
            loss_4 = F.mse_loss(dot_product, zero_vector)
            ...
            loss_5 = F.mse_loss(v1_consist[mask].detach(), v2_consist[mask]) + F.mse_loss(v1_consist[~mask], v2_consist[~mask].detach())

            loss = loss_1 + loss_2 + loss_3 + args.loss_orth * loss_4 + args.loss_consist * loss_5

            loss.backward()
            opt.step()
            preds = logits.max(dim=1)[1]
            preds_vn = logits_vn.max(dim=1)[1]
            preds_fuse = logits_fuse.max(dim=1)[1]
            count += batch_size
        ...
        outstr_2 = 'Train %d, loss vn: %.6f, train acc vn: %.6f, train avg acc vn: %.6f' % (epoch,
                                                                                         ...
                                                                                         metrics.balanced_accuracy_score(
                                                                                             train_true, train_pred_2))
        outstr_3 = 'Train %d, loss fuse: %.6f, train acc fuse: %.6f, train avg acc fuse: %.6f' % (epoch,
                                                                                             train_loss_3 * 1.0 / count,
                                                                                             metrics.accuracy_score(
                                                                                                 train_true,
                                                                                                 train_pred_3),
                                                                                             metrics.balanced_accuracy_score(
                                                                                                 train_true,
                                                                                                 train_pred_3))
        outstr_4 = 'Train %d, orth_loss: %.6f,consist_loss: %.6f' % (epoch, train_loss_4 * 1.0 / count, train_loss_5 * 1.0 / count)
        io.cprint(outstr_1)
        io.cprint(outstr_2)
        io.cprint(outstr_3)
        io.cprint(outstr_4)
        ...
        with torch.no_grad():
            ...
            for data, label in tqdm(test_loader):
                ...
                data = data.permute(0, 2, 1)
                batch_size = data.size()[0]
                logits, logits_vn, logits_fuse, _ = model(data)
                ...
                preds_vn = logits_vn.max(dim=1)[1]
                preds_fuse = logits_fuse.max(dim=1)[1]
                preds_1 = (logits_vn + logits_fuse).max(dim=1)[1]
                preds_2 = (logits + logits_vn + logits_fuse).max(dim=1)[1]
                count += batch_size
                ...
                test_pred_3.append(preds_fuse.detach().cpu().numpy())
                test_pred_4.append(preds_1.detach().cpu().numpy())
                test_pred_5.append(preds_2.detach().cpu().numpy())
            test_true = np.concatenate(test_true)
            test_pred_1 = np.concatenate(test_pred_1)
            test_pred_2 = np.concatenate(test_pred_2)
            test_pred_3 = np.concatenate(test_pred_3)
            test_pred_4 = np.concatenate(test_pred_4)
            test_pred_5 = np.concatenate(test_pred_5)
            ...
            test_acc_fuse = metrics.accuracy_score(test_true, test_pred_3)
            avg_per_class_acc_fuse = metrics.balanced_accuracy_score(test_true, test_pred_3)
            test_acc_1 = metrics.accuracy_score(test_true, test_pred_4)
            avg_per_class_acc_1 = metrics.balanced_accuracy_score(test_true, test_pred_4)
            test_acc_2 = metrics.accuracy_score(test_true, test_pred_5)
            avg_per_class_acc_2 = metrics.balanced_accuracy_score(test_true, test_pred_5)
            if test_acc >= best_test_acc:
                best_test_acc = test_acc
            if test_acc_vn >= best_test_acc_vn:
                best_test_acc_vn = test_acc_vn
            if test_acc_fuse >= best_test_acc_fuse:
                best_test_acc_fuse = test_acc_fuse
            if test_acc_1 >= best_test_acc_1:
                best_test_acc_1 = test_acc_1
            if test_acc_2 >= best_test_acc_2:
            ...
            outstr_2 = 'Test %d, loss vn: %.6f, test acc vn: %.6f, test avg acc vn: %.6f, best test acc vn: %.6f' % (epoch,
                                                                                                         ...
                                                                                                         best_test_acc_vn)
            outstr_3 = 'Test %d, loss fuse: %.6f, test acc fuse: %.6f, test avg acc fuse: %.6f, best test acc fuse: %.6f' % (epoch,
                                                                                                         test_loss_3 * 1.0 / count,
                                                                                                         test_acc_fuse,
                                                                                                         avg_per_class_acc_fuse,
                                                                                                         best_test_acc_fuse)
            outstr_4 = 'Test %d, test acc 1: %.6f, test avg acc 1: %.6f, best test acc 1: %.6f' % (epoch, test_acc_1,
                                                                                              avg_per_class_acc_1,
                                                                                              best_test_acc_1)
            outstr_5 = 'Test %d, test acc 2: %.6f, test avg acc 2: %.6f, best test acc 2: %.6f' % (epoch, test_acc_2,
                                                                                              avg_per_class_acc_2,
                                                                                              best_test_acc_2)
            io.cprint(outstr_1)
            io.cprint(outstr_2)
            ...
            if test_acc_fuse >= best_test_acc_fuse:
                torch.save(model.state_dict(), 'results/cls/%s/models/model_fuse.t7' % args.exp_name)
            if test_acc_1 >= best_test_acc_1:
                torch.save(model.state_dict(), 'results/cls/%s/models/model_1.t7' % args.exp_name)
            if test_acc_2 >= best_test_acc_2:
                torch.save(model.state_dict(), 'results/cls/%s/models/model_2.t7' % args.exp_name)
...
def test(args, io):
    ...
    outstr_3 = 'Test, test acc fuse: %.6f, test avg acc fuse: %.6f, best test acc fuse: %.6f' % (test_acc_fuse, avg_per_class_acc_fuse, test_acc_fuse)
    outstr_4 = 'Test, test acc 1: %.6f, test avg acc 1: %.6f, best test acc 1: %.6f' % (test_acc_1, avg_per_class_acc_1, test_acc_1)
    outstr_5 = 'Test, test acc 2: %.6f, test avg acc 2: %.6f, best test acc 2: %.6f' % (test_acc_2, avg_per_class_acc_2, test_acc_2)
    io.cprint(outstr_1)
    io.cprint(outstr_2)
    io.cprint(outstr_3)
    io.cprint(outstr_4)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint', type=str, default='model_1', metavar='N',
                        choices=['model', 'model_vn', 'model_fuse', 'model_1', 'model_2'],
    ...
    parser.add_argument('--add_layer', type=bool, default=True,
